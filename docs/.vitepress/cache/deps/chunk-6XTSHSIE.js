...
function assignCutValue(t, g, child) {
  ...
  t.edge(child, parent2).cutvalue = calcCutValue(t, g, child);
}
function calcCutValue(t, g, child) {
  var childLab = t.node(child);
  var parent2 = childLab.parent;
  var childIsTail = true;
  var graphEdge = g.edge(child, parent2);
  var cutValue = 0;
  if (!graphEdge) {
    childIsTail = false;
    graphEdge = g.edge(parent2, child);
  }
...
function dfsAssignLowLim(tree, visited, nextLim, v, parent2) {
  var low = nextLim;
...
function updateRanks(t, g) {
  ...
  vs = vs.slice(1);
  forEach_default(vs, function(v) {
    var parent2 = t.node(v).parent, edge = g.edge(v, parent2), flipped = false;
    if (!edge) {
      edge = g.edge(parent2, v);
      flipped = true;
    }
    g.node(v).rank = g.node(parent2).rank + (flipped ? edge.minlen : -edge.minlen);
  });
}
...
function addSubgraphConstraints(g, cg, vs) {
  var prev = {}, rootPrev;
  forEach_default(vs, function(v) {
    var child = g.parent(v), parent2, prevChild;
    while (child) {
      parent2 = g.parent(child);
      if (parent2) {
...
function buildLayerGraph(g, rank2, relationship) {
  var root = createRootNode(g), result2 = new Graph({ compound: true }).setGraph({ root }).setDefaultNodeLabel(function(v) {
    return g.node(v);
  });
  forEach_default(g.nodes(), function(v) {
    var node = g.node(v), parent2 = g.parent(v);
    if (node.rank === rank2 || node.minRank <= rank2 && rank2 <= node.maxRank) {
      result2.setNode(v);
      result2.setParent(v, parent2 || root);
      forEach_default(g[relationship](v), function(e) {
        var u = e.v === v ? e.w : e.v, edge = result2.edge(u, v), weight = !isUndefined_default(edge) ? edge.weight : 0;
...
function sort(entries, biasRight) {
  var parts = partition2(entries, function(entry) {
  ...
  var result2 = { vs: flatten_default(vs) };
  if (weight) {
    result2.barycenter = sum2 / weight;
...
function sortSubgraph(g, v, cg, biasRight) {
  ...
  var result2 = sort(entries, biasRight);
  if (bl) {
    result2.vs = flatten_default([bl, result2.vs, br]);
    if (g.predecessors(bl).length) {
      var blPred = g.node(g.predecessors(bl)[0]), brPred = g.node(g.predecessors(br)[0]);
      if (!has_default(result2, "barycenter")) {
...
lodash-es/lodash.js:
   ...
   *)
*/
//# sourceMappingURL=chunk-6XTSHSIE.js.map
